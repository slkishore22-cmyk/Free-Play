...
def import_playlist():
    ...
    try:
        ...
        print(f"Token body: {token_res.text[:200]}")
        
        token = token_res.json().get('accessToken')
        if not token:
            return jsonify({"success": False, "error": "Could not get Spotify token"}), 500
        ...
        while True:
            res = requests.get(
                ...
                params={
                    'limit': 100,
                    'offset': offset,
                    'fields': 'items(track(name,artists(name),album(images,name),duration_ms)),next',
                    'market': 'IN'
                },
                timeout=15
            )
            
            print(f"Playlist status: {res.status_code}")
    ...
    if not tracks:
        return jsonify({"success": False, "error": "No tracks found"}), 500
